...
<body>
	...
	<div id="wrap">
        ...
        <div>
                  <h1>登录帐号</h1>
        <form id="loginForm" action="login?ref=${ref}" method="post">
          用户名 / 邮箱：<span id="accountIdMsg" class="errorMsg"></span><input id="username" name="accountId" tabindex="1" class="username" type="text" value="${accountId }"/>
          密码：<span id="passwordMsg"  class="errorMsg"></span><input id="password" name="password" tabindex="2" class="password" type="password" value=""/>
          验证码：<input id="checkcode" name="checkcode" tabindex="3" class="checkcode" type="text"/>
          <img id="checkcodeImg" alt="验证码" src="checkcode"/>
          <a href="javascript:;" onclick="document.getElementById('checkcodeImg').src='checkcode?'+(new Date().getTime())">看不清？</a>
          ...
          <input class="loginButton" accesskey="l" value="登 录" tabindex="5" type="button" onclick="submitLoginForm();"/>
        </form>
        </div>
        <div>
